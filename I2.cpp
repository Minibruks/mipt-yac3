...
std::vector<int64_t> Solve(IGraphList& graph) {
    std::vector<int64_t> dists(graph.GetVertexDegree(), kBigNum);
    dists[0] = 0;
    while (true) {
        bool any = false;
        for (uint64_t i = 0; i < graph.list_of_edges_.size(); i++) {
            if (dists[graph.list_of_edges_[i].from] < kBigNum) {
                if (dists[graph.list_of_edges_[i].to] > dists[graph.list_of_edges_[i].from] + graph.list_of_edges_[i].cost) {
                    dists[graph.list_of_edges_[i].to] = dists[graph.list_of_edges_[i].from] + graph.list_of_edges_[i].cost;
                    any = true;
                }
            }
        }
...
int main() {
    ...
    std::vector<int64_t> answer = Solve(graph);
